...
{n:int}
{i:nat | i < n}
(xs: ralist1(a, n), i: int(i)): a // O(log(n))

#symload [] with ralist1_get_at
...
case+ xs of
| ralist1_evn
  (xs1, xs2) => helper(xs1, i/2)
| ralist1_odd
  (x0, xs1, xs2) => helper(xs2, (i-1)/2)
) else // i%2 = 1
(
case+ xs of
| ralist1_evn
  (xs1, xs2) => helper(xs2, i/2)
| ralist1_odd
  (x0, xs1, xs2) => helper(xs1, (i-1)/2)
)
)
)
